(() => {
  ...
  function renderSchedules(list, tourPrice) {
    const sContainer = el('schedules');
    if (list && list.length > 0) {
      sContainer.innerHTML = list.map(s => {
        const user = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')) : null;
        const canBook = user && s.status === 'AVAILABLE' && Number(s.availableSlots || 0) > 0;
        return `
          <div class="card" style="box-shadow:none;border-color:rgba(31,41,51,0.06); padding:18px;">
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-top:12px;">
              ...
              ${canBook ? `<button class="btn btn-sm" onclick="window.openBooking(${s.id}, '${s.startDate}', ${tourPrice})">Book Now</button>` : ''}
            </div>
          </div>
        `;
      }).join('');
  ...
  async function load() {
    ...
    renderSchedules(t.schedules, t.price);
  }
